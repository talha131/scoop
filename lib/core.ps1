...
function shim($path, $global, $name, $arg) {
    ...
    if($relpath -match "^(.\\[\w]:).*$") {
        write-host "`$path = `"$path`"" | out-file $shim -encoding utf8
    } else {
        write-host "`$path = join-path `"`$psscriptroot`" `"$relpath`"" | out-file $shim -encoding utf8
    }

    if($arg) {
        write-host "`$args = '$($arg -join "', '")', `$args" | out-file $shim -encoding utf8 -append
    }
    write-host 'if($myinvocation.expectingInput) { $input | & $path @args } else { & $path @args }' | out-file $shim -encoding utf8 -append

    if($path -match '\.exe$') {
        # for programs with no awareness of any shell
        $shim_exe = "$(strip_ext($shim)).shim"
        cp "$(versiondir 'scoop' 'current')\supporting\shimexe\shim.exe" "$(strip_ext($shim)).exe" -force
        write-host "path = $(resolve-path $path)" | out-file $shim_exe -encoding utf8
        if($arg) {
            write-host "args = $arg" | out-file $shim_exe -encoding utf8 -append
        }
...
function ensure_in_path($dir, $global) {
    ...
    if($path -notmatch [regex]::escape($dir)) {
        write-host "Adding $(friendly_path $dir) to $(if($global){'global'}else{'your'}) path."
...
function remove_from_path($dir,$global) {
    ...
    if($was_in_path) {
        write-host "Removing $(friendly_path $dir) from your path."
