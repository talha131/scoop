...
function install_app($app, $architecture, $global, $suggested) {
    ...
    write-host "Installing '$app' ($version)."
...
function create_shims($manifest, $dir, $global, $arch) {
    ...
    $shims | ?{ $_ -ne $null } | % {
        ...
        write-host "Creating shim for '$name'."
...
function rm_shim($name, $shimdir) {
    ...
    if(!(test-path $shim)) { # handle no shim from failed install
        ...
        write-host "Removing shim for '$name'."
        rm $shim
    }
...
function pre_install($manifest, $arch) {
    ...
    if($pre_install) {
        write-host "Running pre-install script..."
        iex $pre_install
    }
}
...
function show_notes($manifest, $dir, $original_dir, $persist_dir) {
    if($manifest.notes) {
        write-host "Notes"
        write-host "-----"
        write-host (wraptext (substitute $manifest.notes @{ '$dir' = $dir; '$original_dir' = $original_dir; '$persist_dir' = $persist_dir}))
    }
}
